...
"""
VoiceTTSr GUI  —  v1.7.0 (Chatterbox integration)
XTTS v2 + Qwen3-TTS + Chatterbox TTS run in isolated Python envs via subprocess workers.
No ML imports in this file — zero version conflicts.
Run: python voice_cloner_gui.py
GUI deps: pip install pydub pygame numpy

Chatterbox setup (Python 3.11 env):
...
class _TtsWorker:
    ...
    def start(self, on_ready, on_error):
        ...
        # Sanitize environment to prevent cross-contamination if the GUI was launched 
        # from a Conda environment or has global Python paths set.
        for key in ["PYTHONHOME", "PYTHONPATH", "CONDA_PREFIX", "CONDA_PYTHON_EXE", "CONDA_DEFAULT_ENV", "CONDA_PROMPT_MODIFIER"]:
...
class VoiceClonerApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self._chatterbox_worker = _TtsWorker(
            python_exe=_CHATTERBOX_PYTHON,
            script="chatterbox_worker.py",
            on_log=self._worker_log,
            on_status=self._set_status,
        )
        ...
        self._chatterbox_model = None  # True once Chatterbox worker is ready
        ...
        # Chatterbox settings
        self._chatterbox_ref_folder_var = None  # set in _build_folders
        self._chatterbox_exagg_var  = tk.DoubleVar(value=self.config_data.get("cb_exaggeration", 0.5))
        self._chatterbox_cfg_var    = tk.DoubleVar(value=self.config_data.get("cb_cfg_weight",   0.5))
        self._chatterbox_temp_var   = tk.DoubleVar(value=self.config_data.get("cb_temperature",  0.8))
        self._chatterbox_steps_var  = tk.IntVar(   value=self.config_data.get("cb_max_steps",    40))
        self._cb_profile_path       = None
        self._cb_profile_var        = tk.StringVar(value=self.config_data.get("cb_profile_path", ""))
        ...
        os.makedirs("references/chatterbox", exist_ok=True)
    ...
    def _save_config(self):
        try:
            ...
            # chatterbox settings
            d["cb_exaggeration"]      = self._chatterbox_exagg_var.get()
            d["cb_cfg_weight"]        = self._chatterbox_cfg_var.get()
            d["cb_temperature"]       = self._chatterbox_temp_var.get()
            d["cb_max_steps"]         = self._chatterbox_steps_var.get()
            if self._chatterbox_ref_folder_var:
                d["cb_ref_folder"]    = self._chatterbox_ref_folder_var.get()
            if self._cb_profile_path:
                d["cb_profile_path"]  = self._cb_profile_path
            # skyrim settings
    ...
    def _build_backend(self, parent):
        ...
        def _pick(val):
            ...
            is_xtts = val == "xtts"
            is_qwen = val == "qwen"
            is_cb   = val == "chatterbox"
            self._btn_xtts.config(bg=ACCENT  if is_xtts else CARD_BG,
                                   fg=DARK_BG if is_xtts else TEXT_SEC)
            self._btn_qwen.config(bg=ACCENT3 if is_qwen else CARD_BG,
                                   fg=DARK_BG if is_qwen else TEXT_SEC)
            self._btn_cb.config(bg=ACCENT2 if is_cb else CARD_BG,
                                fg=DARK_BG if is_cb else TEXT_SEC)

            subtitle = {
                "xtts":        "  XTTS v2 voice cloning studio",
                "qwen":        "  Qwen3-TTS voice cloning studio",
                "chatterbox":  "  Chatterbox TTS voice cloning studio",
            }
            self._subtitle_lbl.config(text=subtitle.get(val, ""))
            ...
            if is_xtts:
                ...
                self._cb_settings_frame.pack_forget()
                ...
                self._qwen_settings_frame.pack_forget()
                self._enhance_refs_frame.pack_forget()
                self._cb_settings_frame.pack(fill="x")
        ...
        # ── two-row button grid so nothing gets crushed ──────────────────
        row1 = tk.Frame(c, bg=CARD_BG); row1.pack(fill="x")
        row2 = tk.Frame(c, bg=CARD_BG); row2.pack(fill="x", pady=(4, 0))

        self._btn_xtts = self._btn(row1, "XTTS v2  (stable)",
                                    lambda: _pick("xtts"), accent=True, small=True)
        tk.Frame(row1, width=8, bg=CARD_BG).pack(side="left")
        self._btn_qwen = self._btn(row1, "Qwen3-TTS  (higher quality)",
                                    lambda: _pick("qwen"), small=True)
        self._btn_cb   = self._btn(row2, "Chatterbox  (SoTA · emotion control)",
                                    lambda: _pick("chatterbox"), small=True)
        ...
        tk.Label(info,
                 text="Chatterbox: English · ~1.5 GB · emotion control · SoTA quality  "
                      "needs: pip install chatterbox-tts torchaudio  (Python 3.11)",
                 font=("Courier New", 7), bg=CARD_BG, fg=TEXT_MUT,
                 wraplength=420, justify="left").pack(anchor="w")
    ...
    def _build_folders(self, parent):
        ...
        self._chatterbox_ref_folder_var = tk.StringVar(
            value=self.config_data.get("cb_ref_folder", "references/chatterbox"))
        ...
        for label, var, title in [
            ...
            ("Chatterbox References",  self._chatterbox_ref_folder_var,   "Chatterbox Reference folder (1 best clip is used)"),
        ...
        tk.Label(c, text="Tip: Qwen works well with 10-30 diverse reference clips. Chatterbox uses 1 clip — pick your cleanest 10-30s sample.",
    ...
    def _apply_preset(self, name, silent=False):
        ...
        vals = active_presets[name]
        desc = vals[3] if len(vals) > 3 else vals[-1]
        if backend == "xtts":
            temp, speed, rep, topk, topp, desc = vals
            ...
            exagg, cfg, temp, desc = vals
            self._chatterbox_exagg_var.set(exagg)
            self._chatterbox_cfg_var.set(cfg)
            self._chatterbox_temp_var.set(temp)
            for lbl, val in [(self._lbl_cb_exagg, exagg),
                             (self._lbl_cb_cfg,   cfg),
                             (self._lbl_cb_temp,  temp)]:
                lbl.config(text=f"{val:.2f}")
    ...
    def _run_auto_detect(self, wav_files):
        ...
        try:
            ...
            else:  # chatterbox — map energy/pv to exaggeration
                active_presets = CHATTERBOX_PRESETS
                # CB presets use exaggeration (vals[0]) — map pv_n → exaggeration
                feat_exagg = 0.35 + pv_n * 1.2
                best, best_d = "Natural", float("inf")
                for name, vals in active_presets.items():
                    d = abs(vals[0] - feat_exagg)
                    if d < best_d: best_d = d; best = name
                if eng_n < 0.25: best = "Whisper"
                elif eng_n > 0.80 and pv_n > 0.70: best = "Dramatic"
                elif eng_n < 0.30 and rate_n < 0.35: best = "Warm"
    ...
    def _build_settings(self, parent):
        ...
        # ── Chatterbox panel ────────────────────────────────────────────────
        self._cb_settings_frame = tk.Frame(c, bg=CARD_BG)
        self._lbl_cb_exagg = self._slider_row(self._cb_settings_frame, "Exaggeration",
                                               self._chatterbox_exagg_var, 0.25, 2.00, 0.05,
                                               "0.5=neutral · 1.0=expressive · 2.0=dramatic")
        self._lbl_cb_cfg   = self._slider_row(self._cb_settings_frame, "CFG Weight",
                                               self._chatterbox_cfg_var,   0.00, 1.00, 0.05,
                                               "0.5=normal · 0.3=slow ref fix · 0.0=cross-lang")
        self._lbl_cb_temp  = self._slider_row(self._cb_settings_frame, "Temperature",
                                               self._chatterbox_temp_var,  0.05, 1.00, 0.05,
                                               "lower=consistent · higher=varied")
        self._lbl_cb_steps = self._slider_row(self._cb_settings_frame, "Diffusion Steps",
                                               self._chatterbox_steps_var, 10,   80,   5,
                                               "30-50 = fast+good · 80 = max quality · default 40")
        tk.Label(self._cb_settings_frame,
                 text="Tip: Chatterbox uses 1 reference clip (first valid WAV). "
                      "Pick a clean 10-30s sample with no background noise.\n"
                      "Paralinguistic tags work in Turbo: [laugh] [cough] [chuckle] [sigh]",
                 font=("Courier New", 7), bg=CARD_BG, fg=TEXT_MUT,
                 wraplength=400, justify="left").pack(anchor="w", pady=(4, 0))

        # ── CB Load button ─────────────────────────────────────────────────
        tk.Frame(self._cb_settings_frame, bg=BORDER, height=1).pack(fill="x", pady=8)
        cb_load_row = tk.Frame(self._cb_settings_frame, bg=CARD_BG)
        cb_load_row.pack(fill="x")
        self._btn(cb_load_row, "Load Chatterbox Model", self._load_model, accent=True, small=True)
        self._cb_model_lbl = tk.Label(cb_load_row, text="not loaded",
                                       font=("Courier New", 8), bg=CARD_BG, fg=TEXT_MUT)
        self._cb_model_lbl.pack(side="left", padx=8)

        # ── CB Voice Profile ───────────────────────────────────────────────
        tk.Frame(self._cb_settings_frame, bg=BORDER, height=1).pack(fill="x", pady=8)
        tk.Label(self._cb_settings_frame, text="CHATTERBOX VOICE PROFILE",
                 font=("Courier New", 9, "bold"), bg=CARD_BG, fg=ACCENT2).pack(anchor="w", pady=(0, 4))

        cb_prow = tk.Frame(self._cb_settings_frame, bg=CARD_BG)
        cb_prow.pack(fill="x", pady=(0, 4))
        tk.Label(cb_prow, text="Profile (.cbprof)", width=16, anchor="w",
                 font=("Courier New", 9), bg=CARD_BG, fg=TEXT_SEC).pack(side="left")
        tk.Entry(cb_prow, textvariable=self._cb_profile_var, bg=PANEL_BG, fg=TEXT_PRI,
                 insertbackground=TEXT_PRI, relief="flat", bd=4,
                 font=("Courier New", 9)).pack(side="left", fill="x", expand=True, padx=(0, 4))
        self._btn(cb_prow, "Browse", self._browse_cb_profile, small=True)

        self._cb_profile_status = tk.Label(
            self._cb_settings_frame,
            text="No profile — ref WAV scanned each run",
            font=("Courier New", 7), bg=CARD_BG, fg=TEXT_MUT)
        self._cb_profile_status.pack(anchor="w", pady=(0, 6))

        cb_pbtn_row = tk.Frame(self._cb_settings_frame, bg=CARD_BG)
        cb_pbtn_row.pack(fill="x")
        self._save_cb_btn = self._btn(cb_pbtn_row, "Save Profile",
                                       self._save_cb_profile, small=True)
        tk.Frame(cb_pbtn_row, width=8, bg=CARD_BG).pack(side="left")
        self._load_cb_btn = self._btn(cb_pbtn_row, "Load Profile",
                                       self._load_cb_profile, accent=True, small=True)
        tk.Frame(cb_pbtn_row, width=8, bg=CARD_BG).pack(side="left")
        self._btn(cb_pbtn_row, "Clear", self._clear_cb_profile, small=True)
        tk.Label(self._cb_settings_frame,
                 text="Save computes a voice latent from your ref — loads instantly next time.",
                 font=("Courier New", 7), bg=CARD_BG, fg=TEXT_MUT,
    ...
    def _save_cb_profile(self):
        if not self._chatterbox_worker.is_alive():
            messagebox.showerror("No model", "Load Chatterbox model first."); return
        valid_wavs, _ = self._get_ref_wavs()
        if not valid_wavs:
    ...
    def _run_save_cb_profile(self, wav_files, out_path):
        try:
            self._log("Chatterbox: computing voice conditioning profile…", WARNING)
            self._set_status("building CB profile…", WARNING)
            self._chatterbox_worker.send({"action": "save_profile",
                                          "refs": wav_files, "out_path": out_path})
            resp = self._chatterbox_worker.get_response(timeout=120)
            if resp.get("status") == "done_save":
                self._cb_profile_path = out_path
                self._cb_profile_var.set(out_path)
                self.config_data["cb_profile_path"] = out_path
                name = os.path.basename(out_path)
                self.after(0, lambda: self._cb_profile_status.config(
                    text=f"Profile active: {name}", fg=ACCENT2))
                self._log(f"CB profile saved → {name}", ACCENT2)
                self._set_status("CB profile ready", ACCENT2)
            else:
                raise Exception(resp.get("message", "unknown error"))
        except Exception as e:
            self._log(f"Save CB profile failed: {e}", DANGER)
        finally:
            self.after(0, lambda: self._save_cb_btn.config(state="normal"))

    def _load_cb_profile(self):
        ...
        if not os.path.exists(path):
            messagebox.showerror("Not found", f"File not found:\n{path}"); return
        if not self._chatterbox_worker.is_alive():
            # Just store the path — worker will load on next generate
            self._cb_profile_path = path
            self.config_data["cb_profile_path"] = path
            name = os.path.basename(path)
            self._cb_profile_status.config(text=f"Profile queued: {name}", fg=ACCENT2)
            self._log(f"CB profile set: {name} (will load on generation)", ACCENT2)
            return
        self._load_cb_btn.config(state="disabled")
        threading.Thread(target=self._run_load_cb_profile, args=(path,), daemon=True).start()

    def _run_load_cb_profile(self, path):
        try:
            self._log(f"Loading CB profile: {os.path.basename(path)}…", WARNING)
            self._chatterbox_worker.send({"action": "load_profile", "profile_path": path})
            resp = self._chatterbox_worker.get_response(timeout=60)
            if resp.get("status") == "done_load":
                self._cb_profile_path = path
                self.config_data["cb_profile_path"] = path
                name = os.path.basename(path)
                ref_name = resp.get("ref_name", "?")
                self.after(0, lambda: self._cb_profile_status.config(
                    text=f"Profile active: {name}  ({ref_name})", fg=ACCENT2))
                self._log(f"CB profile loaded: {ref_name}", ACCENT2)
                self._set_status("CB profile ready", ACCENT2)
            else:
                raise Exception(resp.get("message", "unknown error"))
        except Exception as e:
            self._log(f"Load CB profile failed: {e}", DANGER)
        finally:
            self.after(0, lambda: self._load_cb_btn.config(state="normal"))

    def _clear_cb_profile(self):
        self._cb_profile_path = None
        self._cb_profile_var.set("")
        self.config_data.pop("cb_profile_path", None)
        self._cb_profile_status.config(
            text="No profile — ref WAV scanned each run", fg=TEXT_MUT)
        self._log("CB profile cleared.", TEXT_MUT)
    ...
    def _build_jobs(self, parent):
        ...
        self._jobs_frame.pack(fill="x", anchor="n")
    ...
    def _export_json(self):
        ...
        jobs = [(var.get().strip(), nv.get().strip())
                for var, _, __, nv, *_rest in self._job_entries
                if var.get().strip()]
        if not jobs:
    ...
    def _load_model(self):
        ...
        def on_ready():
            ...
            if backend == "xtts":
                self._tts_model = True
                self.after(0, lambda: self._model_lbl.config(text="XTTS v2  (worker)", fg=ACCENT2))
            elif backend == "qwen":
                self._qwen_model = True
                self.after(0, lambda: self._model_lbl.config(text="Qwen3-TTS  (worker)", fg=ACCENT3))
            else:
                self._chatterbox_model = True
                self.after(0, lambda: self._model_lbl.config(text="Chatterbox  (worker)", fg=ACCENT2))
                self.after(0, lambda: self._cb_model_lbl.config(text="online", fg=ACCENT2))
    ...
    def _get_ref_wavs(self):
        ...
        if backend == "qwen":
            folder = self.qwen_ref_folder_var.get()
            self.ref_folder_var = self.qwen_ref_folder_var
        elif backend == "chatterbox":
            folder = self._chatterbox_ref_folder_var.get()
            self.ref_folder_var = self._chatterbox_ref_folder_var
        else:
            folder = self.xtts_ref_folder_var.get()
            self.ref_folder_var = self.xtts_ref_folder_var
        wavs   = glob.glob(os.path.join(folder, "*.wav"))
        AudioSegment = _import_pydub()
        valid, skipped = [], []
        for w in wavs:
    ...
    def _generate_all(self):
        ...
        worker_map = {"xtts": self._xtts_worker, "qwen": self._qwen_worker,
                      "chatterbox": self._chatterbox_worker}
        worker = worker_map[backend]
        label_map = {"xtts": "XTTS v2", "qwen": "Qwen3-TTS", "chatterbox": "Chatterbox"}
        if not worker.is_alive():
            messagebox.showerror("No model", f"Load {label_map[backend]} first."); return
    ...
    def _run_generation(self, batches, out_dir):
        ...
        if backend == "xtts":
            ...
            valid_wavs, skipped = self._get_ref_wavs()
            if valid_wavs:
                self._log(f"Chatterbox — using ref: {os.path.basename(valid_wavs[0])}", ACCENT)
            else:
                self._log("Chatterbox — no reference found, using default voice.", WARNING)
        ...
        for b_idx, batch in enumerate(batches, 1):
            ...
            for idx, text, status_lbl, job_mood, custom_filename in batch:
                ...
                if backend == "chatterbox":
                    ...
                    j_temp2, j_speed, j_rep, j_top_k, j_top_p = j_temp, 1.0, 1.0, 50, 1.0
                else:
                    j_temp2, j_speed, j_rep, j_top_k, j_top_p = temp, speed, rep, top_k, top_p
                    j_exagg = 0.5; j_cfg = 0.5  # unused for xtts/qwen
    ...
    def _open_audio_analyzer(self):
        ...
        for label, getter in [
            ...
            ("CB Refs",   lambda: self._chatterbox_ref_folder_var.get()),
        ]:
